...
const mainReplacement = `
      <main className="relative z-10 w-full max-w-6xl mx-auto p-6 md:p-12 flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 min-h-screen">
        ...
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left max-w-lg w-full mt-24 lg:mt-0">
          ...
          {/* Auth Box */}
          <div className="w-full bg-[#22201e] border border-white/10 rounded-2xl p-8 text-left">
            {error && (
        ...
        <div className="flex flex-col gap-4 w-full max-w-md lg:mt-0 mt-8 pb-12 lg:pb-0">
          <div className="bg-[#22201e] border border-white/10 rounded-2xl p-6 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white/10 rounded-lg">
                <Target className="w-5 h-5 text-white" />
              </div>
              <h3 className="text-lg font-bold text-white">Objective</h3>
            </div>
...
const newContent = content.replace(/<main className="relative z-10 w-full h-full min-h-screen flex flex-col items-center justify-center p-6 md:p-12 lg:p-24 pt-32">[\s\S]*<\/main>/, mainReplacement);

fs.writeFileSync('src/components/IntroPage.tsx', newContent);
console.log("Layout patched back to inline");
