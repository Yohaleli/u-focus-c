cat << 'INNER_EOF' > replacement.txt
          {/* Top Right Actions */}
          <div className="absolute top-8 right-8 flex flex-col gap-3 z-50 items-end">
            <button onClick={() => setIsFocusOverlayOpen(false)} className="w-[48px] h-[48px] flex items-center justify-center bg-black/30 backdrop-blur-md border border-white/10 hover:bg-white/10 text-white/60 hover:text-white rounded-full transition-all shadow-lg" title="Exit Immersive Mode">
              <Minimize2 className="w-4 h-4" />
            </button>
            <button onClick={() => setIsTodoOpen(!isTodoOpen)} className={`w-[48px] h-[48px] flex items-center justify-center backdrop-blur-md border border-white/10 text-white/60 hover:text-white rounded-full transition-all shadow-lg ${isTodoOpen ? 'bg-white/20 text-white' : 'bg-black/30 hover:bg-white/10'}`} title="To-Do List">
              <ListTodo className="w-4 h-4" />
            </button>
            
            {isTodoOpen && (
              <div className="mt-2 w-72 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl animate-fade-in text-left">
...
INNER_EOF
sed -i -e '518,525c\' -e "$(cat replacement.txt | sed 's/$/\\/')" src/components/FocusTimer.tsx
